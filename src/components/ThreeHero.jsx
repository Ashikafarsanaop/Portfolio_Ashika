...
export default function ThreeHero() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      coreGeo = new THREE.IcosahedronGeometry(1.6, 1);
      ...
      cardGeo = new THREE.BoxGeometry(0.8, 1.2, 0.08);
      ...
      for (let i = 0; i < 4; i++) {
        ...
        orbitCards.push({ mesh: card, initialAngle: angle, radius });
        mainGroup.add(card);
      }
    ...
    return () => {
      ...
      if (renderer) {
        ...
        if (coreGeo) try { coreGeo.dispose(); } catch {}
        if (cardGeo) try { cardGeo.dispose(); } catch {}
        if (particleGeo) try { particleGeo.dispose(); } catch {}
        const container = containerRef.current;
